...
export class ReservationsController {
  ...
  @Get('stats')
  stats(@Request() req: any) { return this.reservationsService.getStats(req.user?.tenantId); }
  ...
  create(@Body() dto: CreateReservationDto, @Request() req: any, @Ip() ip: string) {
    return this.reservationsService.create(dto, req.user.id, ip, req.user?.tenantId);
  }

  @Put(':id')
  update(@Param('id') id: string, @Body() dto: UpdateReservationDto, @Request() req: any, @Ip() ip: string) {
    return this.reservationsService.update(id, dto, req.user.id, req.user.role, ip, req.user?.tenantId);
  }

  @Delete(':id')
  remove(@Param('id') id: string, @Request() req: any, @Ip() ip: string) {
    return this.reservationsService.remove(id, req.user.id, req.user.role, ip, req.user?.tenantId);
  }
}
