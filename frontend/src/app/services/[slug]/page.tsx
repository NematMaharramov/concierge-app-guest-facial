...
// Fallback images per category slug if no photo uploaded
...
function resolveHeaderImage(category: any): string {
  if (category?.photo) {
    return category.photo.startsWith('http') ? category.photo : `${API_BASE}${category.photo}`;
  }
  return CATEGORY_BG[category?.slug] || 'https://images.unsplash.com/photo-1510414842594-a61c69b5ae57?w=1200&q=90';
}

// ── Image slider inside service cards / modal ────────────────────────────────
  ...
  return (
    <div className={`relative overflow-hidden ${className}`}>
      <div className="flex transition-transform duration-500 ease-out h-full" style={{ transform: `translateX(-${current * 100}%)` }}>
        {images.map((src, i) => (
          <img key={i} src={src} alt={`${alt} ${i + 1}`} className="w-full h-full object-cover flex-shrink-0" style={{ minWidth: '100%' }} />
        ))}
      </div>
      {images.length > 1 && (
        <>
          <button onClick={prev} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 text-white w-7 h-7 rounded-full flex items-center justify-center transition-colors text-xs">‹</button>
          <button onClick={next} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 text-white w-7 h-7 rounded-full flex items-center justify-center transition-colors text-xs">›</button>
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
            {images.map((_, i) => (
              <button key={i} onClick={e => { e.stopPropagation(); setCurrent(i); }} className="w-1.5 h-1.5 rounded-full transition-colors" style={{ background: i === current ? 'white' : 'rgba(255,255,255,0.4)' }} />
            ))}
          </div>
        </>
      )}
...
function FullscreenPreview({ images, startIndex, onClose }: { images: string[]; startIndex: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95" onClick={onClose}>
      <button onClick={onClose} className="absolute top-4 right-4 text-white/70 hover:text-white text-2xl w-10 h-10 flex items-center justify-center">✕</button>
      <img src={images[current]} alt="" className="max-w-[90vw] max-h-[85vh] object-contain" onClick={e => e.stopPropagation()} />
      {images.length > 1 && (
        <>
          <button onClick={e => { e.stopPropagation(); setCurrent(c => (c - 1 + images.length) % images.length); }} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 text-white w-12 h-12 rounded-full flex items-center justify-center text-xl transition-colors">‹</button>
          <button onClick={e => { e.stopPropagation(); setCurrent(c => (c + 1) % images.length); }} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 text-white w-12 h-12 rounded-full flex items-center justify-center text-xl transition-colors">›</button>
          <p className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white/50 text-sm">{current + 1} / {images.length}</p>
        </>
      )}
    </div>
  );
...
export default function CategoryPage() {
  ...
  if (!category) return <div className="min-h-screen flex items-center justify-center"><p>Category not found</p></div>;

  const headerImage = resolveHeaderImage(category);
    ...
    <div className="min-h-screen bg-[#fafaf8]">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/97 backdrop-blur-sm border-b border-charcoal-100">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center gap-4">
          <Link href="/" className="font-display text-lg font-light tracking-[0.15em]" style={{ color: primary }}>RAFFLES SEYCHELLES</Link>
          <span className="text-charcoal-300">/</span>
          <span className="text-xs tracking-widest uppercase text-charcoal-500">{category.name}</span>
        </div>
      </nav>

      {/* ── Category header — full bleed with photo + blur overlay ── */}
      <div className="relative pt-16 h-80 overflow-hidden">
        {/* Background image */}
        <img
          src={headerImage}
          alt={category.name}
        ...
        <div className="relative h-full flex flex-col items-center justify-end pb-10 text-center px-4">
          <h1 className="font-display text-4xl md:text-5xl font-light text-white mb-3" style={{ letterSpacing: '0.06em', textShadow: '0 2px 20px rgba(0,0,0,0.4)' }}>
            {category.name}
          </h1>
          <div className="w-12 h-px mx-auto mb-3" style={{ background: accent }} />
          {category.description && (
            <p className="text-white/80 text-sm max-w-xl mx-auto leading-relaxed" style={{ textShadow: '0 1px 8px rgba(0,0,0,0.5)' }}>
              {category.description}
            </p>
          )}
        </div>
              ...
              <div key={service.id} className="card cursor-pointer group hover:shadow-xl transition-all duration-300 animate-slide-up bg-white" onClick={() => setSelectedService(service)}>
                <div className="relative h-48 overflow-hidden bg-charcoal-100">
                  {serviceImages.length > 0 ? (
                    <ImageSlider images={serviceImages} alt={service.name} className="w-full h-48" />
                  ) : (
                    <div className="w-full h-full bg-cover bg-center" style={{ backgroundImage: `url(${headerImage})` }}>
                  ...
                  {serviceImages.length > 1 && (
                    <div className="absolute top-2 right-2 bg-black/50 text-white text-[10px] px-2 py-0.5 rounded-full pointer-events-none">{serviceImages.length} photos</div>
                  )}
                </div>
                <div className="p-5">
                  <h3 className="font-medium text-charcoal-900 mb-2 leading-snug">{service.name}</h3>
                  {service.description && <p className="text-charcoal-500 text-xs leading-relaxed mb-3 line-clamp-2">{service.description}</p>}
                  <div className="flex items-center justify-between pt-3 border-t border-charcoal-100">
                    {service.priceInfo ? <p className="text-charcoal-900 font-medium text-sm">{service.priceInfo}</p> : <span />}
      ...
      {selectedService && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setSelectedService(null)}>
          <div className="bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            {(() => {
              ...
              return imgs.length > 0 ? (
                <div className="relative">
                  ...
                  <button onClick={() => { setFullscreenImages(imgs); setFullscreenStart(0); }} className="absolute bottom-3 right-3 bg-black/50 hover:bg-black/70 text-white text-[10px] tracking-widest uppercase px-3 py-1.5 transition-colors">
                    ⛶ Full Screen
                  </button>
                </div>
              ) : (
                <div className="h-48 bg-cover bg-center" style={{ backgroundImage: `url(${headerImage})` }} />
              );
            })()}
            ...
            <div className="p-6">
              ...
              {selectedService.description && <p className="text-charcoal-600 text-sm leading-relaxed mb-4">{selectedService.description}</p>}
      ...
      {/* Fullscreen preview */}
      {fullscreenImages && <FullscreenPreview images={fullscreenImages} startIndex={fullscreenStart} onClose={() => setFullscreenImages(null)} />}
