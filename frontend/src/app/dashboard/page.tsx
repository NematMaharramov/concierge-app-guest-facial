...
function AdminStatCard({ label, value, color }: { label: string; value: number | string; color: string }) {
  return (
    <div className="bg-white border border-charcoal-100 p-5 relative overflow-hidden">
...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    getReservations().then(data => setRecent(data.filter((r: any) => r.status !== 'COMPLETED').slice(0, 8)));
  }, []);
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* Stats — 6 cards including Completed */}
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <AdminStatCard label="Completed"    value={stats.completed}   color="bg-blue-500" />
      ...
      <div className="bg-white border border-charcoal-100">
        <div className="px-6 py-4 border-b border-charcoal-100 flex items-center justify-between">
          <h2 className="font-medium text-charcoal-900 tracking-wide">Active Reservations</h2>
          <Link href="/dashboard/reservations" className="text-xs tracking-widest uppercase text-gold-500 hover:text-gold-600 transition-colors">
            View All →
          </Link>
        </div>
        <div className="divide-y divide-charcoal-50">
          {recent.length === 0 ? (
            <p className="text-charcoal-400 text-sm p-6 text-center">No active reservations.</p>
          ) : (
            recent.map(r => (
              <Link key={r.id} href={`/dashboard/reservations`}
...
function ConciergeDashboard({ userName }: { userName: string }) {
  ...
  const todayItems = active.filter(r => { try { return format(new Date(r.dateTime), 'yyyy-MM-dd') === todayStr; } catch { return false; } });
  const pending   = active.filter(r => r.status === 'PENDING');
  const arranged  = active.filter(r => r.status === 'ARRANGED');
  ...
  return (
    <div className="animate-fade-in space-y-6">
      {/* Welcome banner */}
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0e1821 0%, #152433 50%, #0a1c2e 100%)' }}>
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-80 h-80 rounded-full" style={{ background: '#1d8a9a', filter: 'blur(90px)', opacity: 0.18, transform: 'translate(30%, -30%)' }} />
          <div className="absolute bottom-0 left-0 w-56 h-56 rounded-full" style={{ background: '#0e7a88', filter: 'blur(70px)', opacity: 0.14, transform: 'translate(-20%, 20%)' }} />
        </div>
      ...
      {/* Stats — 4 cards including Completed */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Completed', value: completed.length,  color: '#1d4ed8', bg: '#eff6ff' },
        ].map(s => (
          <div key={s.label} className="p-5 border" style={{ background: s.bg, borderColor: `${s.color}22` }}>
            <p className="text-xs tracking-widest uppercase mb-2" style={{ color: `${s.color}99` }}>{s.label}</p>
        ...
        <div className="bg-white border border-charcoal-100">
          <div className="px-6 py-4 border-b border-charcoal-100">
            <h2 className="font-medium text-charcoal-900 tracking-wide flex items-center gap-2">
              ...
              Today's Schedule
              <span className="ml-auto text-xs text-charcoal-400 font-normal">{format(today, 'EEEE d MMM')}</span>
            </h2>
          </div>
          <div className="divide-y divide-charcoal-50">
            {todayItems.map(r => (
              <Link key={r.id} href="/dashboard/reservations"
                ...
                <div className="text-center w-12 flex-shrink-0">
                  <p className="text-sm font-medium text-charcoal-900">{format(new Date(r.dateTime), 'HH:mm')}</p>
                </div>
                ...
                <span className={`text-[10px] px-2.5 py-1 rounded-full font-medium flex-shrink-0 ${statusColors[r.status]}`}>{r.status}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white border border-charcoal-100">
        <div className="px-6 py-4 border-b border-charcoal-100 flex items-center justify-between">
          <h2 className="font-medium text-charcoal-900 tracking-wide">Active Reservations</h2>
          <Link href="/dashboard/reservations" className="text-xs tracking-widest uppercase transition-colors" style={{ color: '#0e7a88' }}>View All →</Link>
        </div>
        {loading ? (
          <div className="p-8 text-center text-charcoal-400 text-sm">Loading…</div>
        ) : recent.length === 0 ? (
          <div className="p-8 text-center">
            <p className="text-charcoal-400 text-sm mb-4">No active reservations.</p>
            <Link href="/dashboard/reservations" className="btn-primary text-xs">+ Create Reservation</Link>
          </div>
        ) : (
          <div className="divide-y divide-charcoal-50">
            {recent.map(r => (
      ...
      <Link href="/dashboard/reservations"
        ...
        <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-white text-lg" style={{ background: '#0e7a88' }}>+</div>
        <div>
